...
public class EventHandler_1_20_R1 implements Listener, MMHEventHandler {
	/** Variables */
	MoreMobHeads mmh;
	...
	@SuppressWarnings({ })
	public EventHandler_1_20_R1(MoreMobHeads plugin, HeadManager headManager) {
		// TODO: Top of code
			...
			if ((entity instanceof Player) && ((entity.getKiller() instanceof Player) || (entity.getKiller() instanceof Creeper))) {
				...
				try{ // REPORT_PLAYER_KILL_PLAYER "Unable to parse Player Death."
					...
					if ((Perms.PLAYERS.hasPermission(theKiller) || mmh.isDev) || (entity.getKiller() instanceof Creeper)) {
						...
						mmh.logDebug("EDE DropIt=" + mmh.DropIt(event, player_Chance, entity.getKiller() ));
						mmh.logDebug("EDE chance_percent.player=" + player_Chance );

						if (mmh.DropIt(event, player_Chance , entity.getKiller())) {
					...
					if ( ( (entity.getLastDamageCause().getCause().equals(DamageCause.BLOCK_EXPLOSION)) || (entity.getKiller() instanceof Creeper) )
						...
						try { // REPORT_PLAYER_KILL_MOB "Unable to parse Mob Kill."
							...
							case "CREEPER":
								...
								if (mmh.DropIt(event, cchance, theKiller)) {
							...
							case "WOLF":
								...
								break;
							default:
								if(Objects.equals(name, "CHICKEN")) { name = "CHICKEN.NORMAL"; }
								if(Objects.equals(name, "COW")) { name = "COW.NORMAL"; }
								if(Objects.equals(name, "PIG")) { name = "PIG.NORMAL"; }
								if(Objects.equals(name, "SNOW_GOLEM")) {
									name = "SNOWMAN";
								}
