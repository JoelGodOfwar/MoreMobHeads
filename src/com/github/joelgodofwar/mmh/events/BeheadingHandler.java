        ...
        try {
            ...
            if (player == playerBedKiller && damagingPlayerUUID != null) {
                ...
                synchronized (playerKills) {
                    ...
                    mmh.logDebug("Added kill for player " + player.getName() + ", entity " + entityName + ", total kills: " + playerKills.size());
                }
            }
            ...
            if (damagingPlayerUUID != null) {
                ...
                } else if (damagingWeapon == null || !damagingWeapon.getType().toString().endsWith("_BED")) {
                    mmh.playerWeapons.remove(damagingPlayerUUID);
                    mmh.entityPlayers.remove(entity.getUniqueId());
                    mmh.logDebug("Cleanup - Removed player " + player.getName() + " and entity " + entityName + " for non-bed kill");
                }
            }
        } catch (Exception exception) {
