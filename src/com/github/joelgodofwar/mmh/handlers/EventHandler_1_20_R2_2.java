...
public class EventHandler_1_20_R2_2 implements Listener, MMHEventHandler {
	/** Variables */
	MoreMobHeads mmh;
	...
	public EventHandler_1_20_R2_2(MoreMobHeads plugin, HeadManager headManager) {
		// TODO: Top of code
			...
			if ((entity instanceof Player) && ((entity.getKiller() instanceof Player) || (entity.getKiller() instanceof Creeper))) {
				...
				try{ // REPORT_PLAYER_KILL_PLAYER "Unable to parse Player Death."
					...
					if ((entity.getKiller().hasPermission("moremobheads.players") || mmh.isDev) || (entity.getKiller() instanceof Creeper)) {
						...
						if (drop_It) {
							...
							if (!isCanceled) {
								mmh.playerGiveOrDropHead(killer, head);
								mmh.logDebug("EDE " + ((Player) entity).getDisplayName() + " Player Head Dropped");
								if (player_announce_enabled) {
									beheading.announceBeheading(entity, entityName, killer, player_announce_display);
								}
							}
						}
						return;
				...
				|| (entity.getLastDamageCause().getCause().equals(DamageCause.BLOCK_EXPLOSION))) {
					...
					if ( ( (entity.getLastDamageCause().getCause().equals(DamageCause.BLOCK_EXPLOSION)) || (entity.getKiller() instanceof Creeper) )
							|| (theKiller.hasPermission("moremobheads.mobs")  || mmh.isDev) ) {

						if ( ((theKiller instanceof Creeper) || (theKiller.hasPermission("moremobheads.nametag")  || mmh.isDev) ) && (isNametag != null)) {
								...
								if ((entity.getCustomName() != null) && !(entity.getCustomName().contains("jeb_")) && !(entity.getCustomName().contains("Toast"))) {
									...
									if ((entity instanceof Skeleton) || (entity instanceof Zombie) || (entity instanceof PigZombie)) {
										...
										boolean on_blacklist = mmh.config.getString("head_settings.player_heads.blacklist.player_head_blacklist", "")
												.toLowerCase().contains(entity.getCustomName().toLowerCase());
										//**  */
										if (mmh.DropIt(event, chanceConfig.getDouble("chance_percent.named_mob", 10.0), theKiller)) {
								...
								if (mmh.chance25oftrue()) { // chance50oftrue()
									...
									if ( mmh.DropIt( event, mhd.getChance(), theKiller ) ) {
										...
										if(HeadUtils.containsHead(Drops)) {
											mmh.logDebug("EDE " + name + " Head Dropped");
											if (mob_announce_enabled) {
												beheading.announceBeheading(entity, mmh.langName.getString(mhd.getLangName(), mhd.getDisplayName()).replace(" Head", ""), theKiller,
														mob_announce_display);
											}
										}
									}
