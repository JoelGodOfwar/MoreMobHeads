		...
		if(checkchanceConfigversion != null){
			if(!checkchanceConfigversion.equalsIgnoreCase(chanceVersion)){
				...
				try {
					...
					reporter.reportDetailed(this, Report.newBuilder(PluginLibrary.REPORT_CHANCE_COPY_ERROR).error(exception));
				}
		...
		try{
			...
			if(blockHeads.getInt("blocks.number", 0) == 0) {
				try {
					...
					reporter.reportDetailed(this, Report.newBuilder(PluginLibrary.REPORT_BLOCKHEAD_LOAD_ERROR).error(exception));
				}
			}
			int numOfCustomTrades = blockHeads.getInt("blocks.number", 0) + 1;
