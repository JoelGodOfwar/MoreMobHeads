...
public class GiveHeadCommand {
	...
	public void execute(Player player) {
		...
		InventoryGUI gui = new InventoryGUI("Select Head Type", headTypes, (selectedPlayer, selectedItem) -> {
			if ((selectedItem == null) || !selectedItem.hasItemMeta()) {
				CoreUtils.warn("Selected item is null or has no meta in Select Head Type GUI");
				return;
			}
			...
			if (headType != null) {
				CoreUtils.log("Player " + selectedPlayer.getName() + " selected head type: " + headType);
				pendingHeadTypes.put(selectedPlayer, headType);
				openTargetSelectionGui(selectedPlayer);
			} else {
				CoreUtils.warn("Unknown head type selected: " + displayName);
			}
		});
	...
	private void openTargetSelectionGui(Player player) {
		...
		if (onlineHeads.isEmpty()) {
			...
			CoreUtils.warn("No targets available for " + player.getName() + " in givehead command");
			return;
		}
		...
		if (senderHead == null) {
			...
			CoreUtils.warn("Sender head not found for " + player.getName() + " in givehead command");
			return;
		}
		...
		targetGui.open(player, (selectedPlayer, selectedHead) -> {
			if ((selectedHead != null) && selectedHead.hasItemMeta()) {
				...
				if (target == null) {
					...
					CoreUtils.warn("Target not found for " + selectedPlayer.getName() + " in givehead command");
					return;
				}

				// Store the target player for the next step
				pendingTargets.put(selectedPlayer, target);
				CoreUtils.log("Player " + selectedPlayer.getName() + " selected target: " + target.getName());
				...
				if (headType == null) {
					...
					CoreUtils.warn("Head type not found for " + selectedPlayer.getName() + " in givehead command");
					return;
				}
	...
	private void giveHead(Player player, ItemStack head) {
		...
		if (target != null) {
			...
			CoreUtils.log("Player " + player.getName() + " gave head to " + target.getName());
		} else {
			player.sendMessage("§cTarget player is no longer online.");
			CoreUtils.warn("Target not found for " + player.getName() + " in givehead command");
		}
	}
	...
	public void loadPlayerHead(Player player) {
		...
		CoreUtils.log("Loaded head for player: " + player.getName());
	}
